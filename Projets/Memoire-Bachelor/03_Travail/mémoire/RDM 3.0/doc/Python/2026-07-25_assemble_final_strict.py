...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
        html, body { margin: 0; padding: 0; background-color: #0A0C1A; color: var(--text); font-family: 'Montserrat', sans-serif; scroll-behavior: smooth; }
        .page {
        ...
        p, li { font-size: 11pt; line-height: 1.6; text-align: justify; margin-bottom: 15px; font-weight: 300; }
        ...
        table { width: 100%; border-collapse: collapse; margin: 25px 0; font-size: 8.5pt; }
        ...
        /* Clearfix for overflowing content */
        .content-body {
            /* Instead of absolutely fixed heights killing content, let it overflow but warn in print */
        }
    </style>
</head>
<body>
    ...
    <div class="page" id="p01">
        ...
        <div class="content-body cover-content">
            ...
            <div style="display: flex; gap: 40px; margin-bottom: 40px;">
                <img src="C:/Users/julien/OneDrive/Bureau/geminicli/super-agent-consulting/mémoire/RDM 3.0/charte graphique/image Happy House.png" alt="Happy House" style="height: 80px; object-fit: contain;">
                <img src="C:/Users/julien/OneDrive/Bureau/geminicli/super-agent-consulting/mémoire/RDM 3.0/charte graphique/rocket logo 2.png" alt="Rocket School" style="height: 80px; object-fit: contain;">
            </div>
...
def chunk_content(html_content, target_words_per_page=220):
    """
    Split the HTML content into pages of roughly equal length, respecting block boundaries.
    Target words lowered to 220 to ensure no overflow on fixed 297mm height pages.
    """
    pages = []
    current_page_content = ""
    current_word_count = 0
    
    # Split by major HTML block tags to avoid cutting mid-paragraph
    ...
    for block in blocks:
        ...
        # Force H1 to always start a new page unless it's the very beginning
        if block.startswith('<h1') and current_page_content.strip() != "":
            pages.append(current_page_content)
            current_page_content = block
            current_word_count = words_in_block
            continue
            
        # If adding this block exceeds our limit, start a new page
        if current_word_count + words_in_block > target_words_per_page and current_word_count > 0:
            pages.append(current_page_content)
            current_page_content = block
            current_word_count = words_in_block
        else:
...
for i, file_path in enumerate(FILES_TO_MERGE):
    if os.path.exists(file_path):
        ...
        chunks = chunk_content(html_segment)
    ...
    <div class="page"{page_id_str}>
        ...
        <div class="footer"><a href="#p02" class="back-btn">◈ RETOUR AU PILOTAGE</a><div>{os.path.basename(file_path).replace('.md','').replace('-',' ').title()[:40]}</div><div class="page-num">{page_counter:02d}</div></div>
    </div>
"""
...
print(f"Succès ! Le fichier final a été généré avec structure de pages stricte : {OUTPUT_FILE}")