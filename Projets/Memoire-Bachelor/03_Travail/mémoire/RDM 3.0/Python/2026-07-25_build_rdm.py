...
chapitres = [
    ...
    ("IV. CONFRONTATION AU TERRAIN (Chapitre 4)", 16),
    ("V. CHOIX D'ACQUISITION & PIVOT (Chapitre 5)", 19),
    ("VI. RÉTENTION & PREUVE DE VALEUR (Chapitre 6)", 22),
    ("VII. PLAN D'ACTION & BILAN (Chapitre 7)", 25),
    ("VIII. CONFORMITÉ IA (Chapitre 8)", 29),
    ("IX. ANNEXES STRATÉGIQUES & PREUVES", 30)
]
...
# --- CHAPITRE 4 : RECHERCHE TERRAIN ---
pages.append({
    "section": "CHAPITRE 4 : RECHERCHE TERRAIN",
    "content": """
        <h1>Chapitre 4 — Confrontation au terrain et analyse des usages</h1>
...
pages.append({
    ...
    "content": """
        ...
        <div class="expert-note">
            ...
            Les signaux recueillis sur le terrain (besoin d'accompagnement, rejet de la prospection numérique agressive) vont directement conditionner nos choix d'acquisition. C'est à la lumière de ces constats que la stratégie initiale d'automatisation sera confrontée aux réalités de la gouvernance, aboutissant à un pivot stratégique majeur.
        </div>
    """
})

# --- CHAPITRE 5 : CHOIX D'ACQUISITION & PIVOT ---
pages.append({
    "section": "CHAPITRE 5 : PIVOT STRATÉGIQUE",
    "content": """
        <h1>Chapitre 5 — Choix d'acquisition et pivot stratégique</h1>
        <p>Face à la nécessité de réduire le coût d'acquisition client (CAC) et d'améliorer l'efficience commerciale, une première réflexion a été menée autour de l'automatisation des processus. Ce chapitre expose cette démarche initiale, les arbitrages qui ont suivi, et la réorientation de la stratégie vers un modèle d'événementiel premium, cohérent avec les attentes du terrain (Chapitre 4) et les exigences de la direction.</p>
...
pages.append({
    "section": "CHAPITRE 5 : PIVOT STRATÉGIQUE",
    "content": """
        <h2>Le blocage interne et l'ajustement stratégique</h2>
        <p>Bien que ce scénario d'automatisation présentât un intérêt financier théorique en rationalisant le temps de prospection, son déploiement a été invalidé par la direction lors de sa présentation. La gouvernance de Happy House a estimé que l'utilisation de campagnes d'emailing sortantes (Cold Emailing) présentait un risque pour le positionnement de la marque, craignant une assimilation à du démarchage automatisé de masse.</p>
        
        <p>Ce refus s'alignait d'ailleurs de manière empirique avec les constats de "fatigue numérique" relevés lors de notre étude terrain. Cette convergence entre la politique de marque et les attentes du marché a dicté l'arbitrage stratégique final.</p>

        <h2>Le pivot : vers une acquisition événementielle</h2>
        <p>L’hypothèse d’automatisation sortante a donc été abandonnée. Ce blocage a conduit à redéfinir la stratégie d'acquisition vers un modèle "Inbound" et événementiel. L'effort d'acquisition a été réalloué vers la création de points de rencontre plus qualitatifs.</p>
    """
})
pages.append({
    "section": "CHAPITRE 5 : PIVOT STRATÉGIQUE",
    "content": """
        <p>La nouvelle stratégie repose sur un cycle de vente qualifié de "sélection", articulé autour de webinaires thématiques et d'Afterworks régionaux. Cette approche vise à réunir des prospects ciblés dans un environnement valorisant, souvent au sein d'un établissement déjà membre, permettant de s'appuyer sur la preuve sociale et l'échange entre pairs plutôt que sur une démarche commerciale directe.</p>
        ...
        <div class="expert-note">
            ...
            L'acquisition étant désormais réorientée vers un modèle événementiel qualitatif, il est impératif de sécuriser la base existante. Le besoin de "Preuve Sociale" et de simplicité (identifié sur le terrain) justifie la conception d’un outil de preuve de valeur pour soutenir la rétention.
        </div>
    """
})
...
pages.append({
    ...
    "content": """
        ...
        <p>Dans un processus opérationnel stabilisé, cet outil est conçu pour être utilisé comme support principal lors de revues trimestrielles menées en visioconférence avec l'adhérent. Cette approche de conseil personnalisé permet de maintenir un point de contact qualitatif, de mesurer l'impact de la strategy d'achat, et d'ajuster les leviers de distribution si nécessaire, réaffirmant ainsi la valeur de l'accompagnement de Happy House.</p>
    """
})
pages.append({
    "section": "CHAPITRE 6 : RÉTENTION & PREUVE DE VALEUR",
    "content": """
        ...
        <p>Au-delà de son rôle dans la fidélisation, ce type de livrable chiffré, une fois anonymisé, s'intègre naturellement à la nouvelle stratégie d'acquisition événementielle détaillée au chapitre précédent. Il constitue une base argumentaire objective lors des événements de présentation pour répondre avec transparence aux interrogations relatives au coût de la cotisation <em>(Cf. Annexe 4 : Comparatif Performance & ROI)</em>.</p>
    """
})
pages.append({
    "section": "CHAPITRE 6 : RÉTENTION & PREUVE DE VALEUR",
...
pages.append({
    ...
    "content": """
        ...
        <p>Le plan de déploiement est structuré sur un cycle de préparation (M1-M6) et d'exécution événementielle (M7-M12), intégrant le pivot stratégique abordé au Chapitre 5.</p>
...
pages.append({
    ...
    "content": """
        ...
        <p>Le pilotage de la stratégie événementielle repose sur deux indicateurs principaux, dont les méthodes de calcul sont formalisées afin de garantir un suivi objectif <em>(Cf. Annexe 8 : Note de Cadrage Force de Vente)</em>.</p>
        ...
        <p>Le traitement des écarts de performance fait l'objet d'un protocole itératif strict :</p>
        <ul>
...
pages.append({
    ...
    "content": """
        ...
        <p>L'efficacité du modèle réseau s'illustre factuellement par l'analyse du cas d'un membre Premium (Domaine de la Durentie). Ce cas d'usage sert de référence pour illustrer la valeur objectivée recherchée par le prototype de reporting.</p>
        
        <p>Le cycle initial de conversion de ce prospect a été d'environ cinq mois, témoignant de la difficulté d'engagement sans preuve de valeur préalable. À la suite de l'adhésion (impliquant une cotisation de 360 € TTC), l'utilisation des accords de la centrale d'achats a généré une économie mesurable de l'ordre de 4 120 € sur les postes de coûts opérationnels de l'établissement.</p>
...
pages.append({
    ...
    "content": """
        ...
        <p>Cette mission d'alternance a constitué un environnement d'apprentissage particulièrement exigeant, imposant de concilier des recommandations stratégiques avec la réalité de la gouvernance d'entreprise. L'abandon forcé des processus d'automatisation (Outbound) a représenté un point d'inflexion formateur : il a fallu accepter l'échec d'un modèle technique pertinent sur le papier pour construire une solution de contournement (l'événementiel) acceptable par la direction.</p>
        
        <p>Ce changement de paradigme a favorisé une évolution significative de ma posture professionnelle. Au-delà des compétences techniques initialement visées, la mission a nécessité le développement de compétences transversales : accompagnement au changement, formation d'un collaborateur (SDR) aux techniques de qualification, et capacité à argumenter des arbitrages complexes.</p>
        
        <p>La principale compétence acquise n'a pas été uniquement technique, mais politique : comprendre qu'une solution algorithmique parfaite échoue si elle n'emporte pas l'adhésion de la direction. Cette expérience valide une transition d'une logique de prospection vers une fonction de pilotage du développement, où la maîtrise de l'analyse (mesure du ROI, suivi du CPA) sert avant tout à sécuriser la prise de décision stratégique de la direction.</p>
