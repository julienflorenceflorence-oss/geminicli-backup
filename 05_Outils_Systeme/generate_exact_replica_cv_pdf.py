...
class CleanPerfectCVPDF:
    ...
    def draw_cv(self):
        ...
        # --------------------------------------------------------
        # MAIN CONTENT (Right Column, x=72mm to 202mm)
        # --------------------------------------------------------
        ...
        # Subtitle (Exact command from user)
        c.setFillColor(text_white)
        c.setFont("Helvetica-Bold", 8.5)
        c.drawString(72*mm, 272*mm, "| DIRECTEUR D'HOTEL HYBRIDE & MANAGER")
        
        # Presentation Text (Exact text command from user)
        ...
        summary_y = 264*mm
        for w in words:
            if c.stringWidth(line + " " + w, "Helvetica", 7.4) < 127*mm:
                line += (" " if line else "") + w
            else:
        ...
        c.save()
        print(f"✅ Nouveau PDF 100% Propre & Sans Superposition généré : {self.filename}")
...
if __name__ == "__main__":
    ...
    builder = CleanPerfectCVPDF(out_file)
    builder.draw_cv()
    
    # Save to Desktop paths
    ...
    builder2 = CleanPerfectCVPDF(desktop_jost)
    builder2.draw_cv()
    builder3 = CleanPerfectCVPDF(desktop_root)
    builder3.draw_cv()
